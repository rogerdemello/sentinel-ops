...
def series(
    ...
    points = get_repository().series(service_id, metric, limit=limit)
    return {
...
@router.get("/events")
def events(limit: int = Query(60, ge=1, le=500), service_id: str | None = None) -> dict:
    items = get_repository().list_events(limit=limit, service_id=service_id)
